...
class SolarDataLoader:
    ...
    def create_price_profile(self, length: int, seed: int = 42) -> pd.DataFrame:
        """
        Create electricity price profile with time-of-use structure
        """
        np.random.seed(seed)
        ...
        # Time-of-use pricing
        # Off-peak (0-6): $0.05/kWh
        # Mid-peak (6-17, 21-24): $0.10/kWh
        # On-peak (17-21): $0.18/kWh
        ...
        price += np.random.normal(0, 0.005, length)
        price = np.maximum(0.03, price)  # Minimum price
...
def main():
    ...
    print(f"\nAverage Price: ${price_profile['price'].mean():.3f}/kWh")
    print("="*60)
