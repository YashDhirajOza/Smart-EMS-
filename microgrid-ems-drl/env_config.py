...
class BatteryConfig:
    ...
    # Cost parameters
    degradation_cost_per_kwh: float = 0.15  # $/kWh throughput cost
...
class GridConfig:
    ...
    # Emissions parameters (kg CO2 per kWh)
    emission_factor_base: float = 0.45  # Base grid emission factor
    emission_factor_peak: float = 0.65  # Peak time emission factor
    emission_factor_offpeak: float = 0.35  # Off-peak emission factor
...
class RewardConfig:
    ...
    alpha: float = 0.05  # Emission weight ($/kg CO2)
    beta: float = 0.5  # Degradation weight
    gamma: float = 1000.0  # Reliability penalty weight (MUST BE VERY LARGE)
    
    # Cost parameters
    revenue_export_multiplier: float = 0.8  # Export price as fraction of import price
    
    # Reliability penalties
    unmet_demand_penalty_per_kwh: float = 10.0  # $/kWh for unmet demand
    safety_violation_penalty: float = 1000.0  # Large penalty for safety violations
...
def print_config_summary():
    ...
    print(f"\nReward Weights: α={REWARD.alpha}, β={REWARD.beta}, γ={REWARD.gamma}")
